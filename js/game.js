...
export class Game{
 constructor(canvas,ui,onLose){
  ...
  this._resetLevel(true);this._last=0;this._time=0;
 }
 _makeLevels(){
  const levels=[];
  for(let i=0;i<8;i++){
 ...
 _update(dt){
  ...
  for(const pl of lvl.platforms){
   ...
   if(prevBottom<=pl.y+2&&p.vy>=0){p.y=pl.y-p.h;p.vy=0;p.onGround=true}
 ...
 _lose(){if(!this.running)return;this.running=false;this.onLose()}
 afterQuiz(ok){if(ok){this.correct++;this.player.invuln=2.0}else{this.wrong++;this._resetLevel(false)}this.running=true;this._last=performance.now();this._syncUI();requestAnimationFrame(t=>this._loop(t))}
