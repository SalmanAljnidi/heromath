...
export class Game{
  ...
  _makeLevels(n){
    const themes=[
      // day
      {mode:'day', top:'#7dd3fc', bottom:'#dbeafe', hill:'#1d4ed8', glow:'#38bdf8', stars:false, clouds:true},
      // dawn (morning twilight)
      {mode:'dawn', top:'#fda4af', bottom:'#60a5fa', hill:'#1e40af', glow:'#fb7185', stars:false, clouds:true},
      // dusk (sunset)
      {mode:'dusk', top:'#fb7185', bottom:'#1e3a8a', hill:'#312e81', glow:'#f59e0b', stars:false, clouds:true},
      // night (boss)
      {mode:'night', top:'#0b1022', bottom:'#02040a', hill:'#05070f', glow:'#a78bfa', stars:true, clouds:false},
    ];
    const lvls=[];
    for(let i=0;i<n;i++){
      ...
      let theme;
      // sky progression: Day -> Dawn -> Dusk -> Day ... (Boss is Night)
      if(i===n-1){
        theme=themes[3];
      }else{
        const phase = Math.floor(i/3)%3; // 0 day,1 dawn,2 dusk
        theme=themes[phase];
      }
