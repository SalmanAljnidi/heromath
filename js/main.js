...
function syncNameUI(){
  ...
  if(game && game.setPlayerName) game.setPlayerName(playerName);
}
syncNameUI();
...
function openName(){
  ...
  if(nameInput){ nameInput.value=playerName||''; nameInput.focus(); }
}
function closeName(){ if(nameOverlay) nameOverlay.classList.remove('show'); }
...
if(btnChangeName) btnChangeName.addEventListener('click', ()=>{ openName(); });

// عند أول فتح: اطلب الاسم فقط إذا ما هو موجود
if(!playerName){ openName(); }
