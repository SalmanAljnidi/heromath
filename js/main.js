...
// show name prompt if needed on load
requireName();

// track best level while playing
setInterval(()=>{
  const current = game.levelIndex+1;
  if(current>bestLevel){
    bestLevel=current;
    localStorage.setItem('heroMathBestLevel', String(bestLevel));
    syncTopBar();
  }
}, 400);
